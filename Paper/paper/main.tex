...
convective instability --- convective available potential energy (CAPE) and
convective inhibition (CIN) --- with a CRPS skill over climatology excluding
...
\begin{enumerate}[leftmargin=1.6em,itemsep=1pt]
  ...
  threshold- and quantile-weighted continuous ranked probability score (CRPS),
...
standard automatic-relevance-determination (ARD) squared-exponential kernel acting
...
distribution with the CRPS and its
...
\caption{Reliability diagrams for the exceedance probability ($95$th, $h{=}1$):
\textbf{(a)} London, \textbf{(b)} Paris. In both cities the logistic regression
and the GP--EVT hurdle track the diagonal (calibrated) and spread their
predictions (informative); the zero-inflated log-normal is systematically
overconfident (its predicted exceedance probabilities exceed the observed
frequencies), despite attaining nominal conformal coverage by construction.
Climatology is a single point at the base rate. (Paris is noisier at high
predicted probabilities, where few days fall.)}
...
single runs. The same decay and flat coverage hold for Paris
(Figure~\ref{fig:horizon}c,d).

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_horizon}
\caption{Seed-averaged horizon sweep on the $95$th-percentile index
(GP--EVT hurdle) for both cities: \textbf{(a,b)} London, \textbf{(c,d)} Paris.
\textbf{(a,c)} Occurrence PR-AUC decays toward the prevalence floor
(dashed) as the forecast horizon grows; the $+$ERA5 driver gain is clearest at
$h=1$ and fades into seed noise by $h=3$.
\textbf{(b,d)} Conformal coverage stays at the nominal $0.90$ across all horizons
...
sophistication bought little, and a modest Gaussian process is about as
flexible as this low-signal problem can bear. Where Gaussian processes would
