...
%
%  Predictability assessment + evaluation methodology; figures built by
%  figures/make_figs.py and figures/make_horizon_fig.py.
...
A calibration-aware assessment for London and Paris}
\author{Olatunji Johnson and Isqeel Ogunsola\\[0.3em]
\small Department of Mathematics, University of Manchester, Manchester, UK\\[0.2em]
...
\begin{abstract}
\noindent
How predictable are rare precipitation extremes, and how should that predictability be measured? We address both questions using thirty years (1989--2018) of a standardised precipitation-extreme index for London (maritime) and Paris (more continental), with strictly antecedent ERA5 drivers. We argue rare-event forecasts must be judged by \emph{calibration-aware proper scores} against climatology, and assemble a protocol (Brier decomposition, threshold-weighted CRPS skill, bootstrap intervals, multi-split checks) guarding against the single-seed and quantile-selection pitfalls that inflate apparent skill. Across a bake-off from logistic regression to a Gaussian-process extreme-value hurdle, we find, consistently in both cities, that the Gaussian-process hurdle is the most skilful full-distribution forecaster on the tail, while simple calibrated models match it on occurrence, and the choice of response distribution matters as much as model class. Occurrence is weakly but genuinely predictable from antecedent convective instability and circulation, and the magnitude of the largest events is weakly but robustly predictable from convective instability, with a CRPS skill over climatology excluding zero across four successive splits per city. The effects are small, statistically robust, and physically plausible, with replication across two cities offering modest evidence against a location-specific artefact.\end{abstract}
...
form does not guarantee conditional or tail-specific coverage --- the effective
resolution of very high quantiles is limited by the size of the calibration set
--- and assumes exchangeability, which is generally violated in daily time series
due to temporal dependence and non-stationarity
\citep{gibbs2021adaptive,pasche2025extreme}.

A further, orthogonal limitation cuts across these tools: many two-part (hurdle)
forecasting approaches estimate occurrence and intensity in separate stages, so
that inference about extremes relies primarily on the relatively small number of
exceedances. This separation can limit the extent to which information contained
in the abundant non-extreme data informs tail inference, particularly when common
latent processes govern both occurrence and intensity.
...
\begin{enumerate}[leftmargin=1.6em,itemsep=1pt]
  \item A calibration-aware evaluation protocol for rare-event
  precipitation forecasting: Brier reliability/resolution decomposition,
  threshold-weighted continuous ranked probability score (CRPS),
  skill scores against climatology,
  ...
  \item A rigorous predictability assessment, replicated in two
  ...
  instability, with a CRPS skill over climatology whose moving-block
  bootstrap interval excludes zero across four successive train/test splits in
  ...
  \item A methodological finding on what matters: across the bake-off, a
  ...
  extended generalised Pareto distribution is well behaved), and conformal coverage, though attained by
...
Generalised Pareto distribution (GPD) \citep{pickands1975,davison1990}, the framework
we adopt for the intensity component; the block-maxima approach uses the
Generalised Extreme Value (GEV) distribution \citep{coles2001evt}. EVT is widely
...
map in deep kernel learning \citep{wilson2016deepkernel}, though as we find below such added depth is not warranted on our low-signal, single-site
...
equation. To our knowledge, this dependence has not been exploited within a GP--EVT \emph{forecasting} model, where intensity is observed only on the rare extreme days, making it natural to borrow strength from the abundant occurrence data. We implement and test this approach and report (in the Supplementary Material) that, at the current data volume, the coupling is only weakly identified and has little empirical impact.
...
threshold-weighted scoring rules focus evaluation on the tail
...
a compact Gaussian-process extreme-value hurdle is the most \emph{skilful} full-distribution forecaster in both cities, that simple calibrated models are competitive, and that conformal coverage alone is an unreliable guide to forecast quality (Section~\ref{sec:discussion}).  This emphasis on rigorous
...
computed for two European cities, {London} and {Paris},
...
We take the {$95$th-percentile index as the primary target}: its $417$
...
Section~S1 (Table~S1). Unless stated, results are for the $95$th percentile.
...
matter.
Models fall into two groups: \emph{occurrence-only classifiers} (which estimate only the exceedance probability~$\pi_{t+h}$, and are not scored on the full distribution) and \emph{full-distribution models} (which output the complete CDF $F_{t+h}$ and are additionally evaluated with the twCRPS of Section~\ref{sec:metrics}).
...
\emph{stationary peaks-over-threshold} (POT) model adds a Generalised Pareto tail \citep{davison1990}
...
isotonic-calibrated gradient-boosted-tree classifier (XGBoost) \citep{chen2016xgboost,friedman2001boosting}, with tree scores mapped to probabilities by isotonic calibration \citep{zadrozny2002calibration}.
...
\end{equation}
with $\mathrm{logit}(p_0(\bx_t)) = \mathbf{w}_0^\top\bx_t$ and the distribution parameters $\bm\theta(\bx_t)$, modelled through parameter-specific inverse link functions applied to linear predictors,
ensuring that the required parameter constraints (e.g. positivity of scale parameters) are satisfied.
The Generalised Pareto distribution is
\begin{equation}\label{eq:gpd}
H(y;\sigma,\xi) = 1-\left(1+\xi y/\sigma\right)_{+}^{-1/\xi}\ (\xi\neq0),\quad
1-e^{-y/\sigma}\ (\xi=0),\quad y\ge0,
\end{equation}
with $(a)_+=\max(a,0)$ and $\xi$ governing tail heaviness. For $F_+$ we compare three choices: (i)~a \emph{log-normal}, (ii)~a \emph{gamma}, and (iii)~the \emph{extended GPD} (eGPD) \citep{naveau2016egpd} --- the extreme-value-motivated choice ---
...
contender is a two-part hurdle whose exceedance intensity component is a GPD with a covariate-dependent scale parameter,
\begin{equation}\label{eq:hurdle-cdf}
F_{t+h}(z) = (1-\pi_{t+h}) + \pi_{t+h}\,H(z-u;\sigma_{t+h},\xi),\qquad z>u,
\end{equation}
with $\mathrm{logit}(\pi_{t+h})=g_\pi(\bx_t)$ and 
$\sigma_{t+h}= \log \{ 1 + \exp(g_\sigma(\bx_t)) \}$ driven by latent functions $g_\pi,
g_\sigma$ on which we place Gaussian-process priors --- a single-layer GP with a
standard automatic-relevance-determination (ARD) squared-exponential kernel \citep{rasmussen2006gpml} acting
...
\emph{a priori} and {not tuned by cross-validation}; a reviewer could ask
...
low-signal regime. We therefore adopt the following protocol. Its individual
ingredients are standard; our contribution is their disciplined integration into
a single artefact-resistant assessment for a low-signal, rare-event regime --- in
particular, pairing calibration-aware proper scores with skill against climatology
and multi-split robustness, and demonstrating that distribution-free conformal
coverage, though guaranteed, is not on its own evidence of forecast quality.

\paragraph{Occurrence: Brier decomposition and skill.} We assess the exceedance
probability with the Brier score, whose reliability/resolution/uncertainty
decomposition \citep{murphy1973},
\begin{equation}\label{eq:brier}
\mathrm{BS} = \underbrace{\frac{1}{n}\sum_{k} n_k\,(\bar p_k-\bar o_k)^2}_{	\text{reliability}}
\;-\; \underbrace{\frac{1}{n}\sum_{k} n_k\,(\bar o_k-\bar o)^2}_{	\text{resolution}}
\;+\; \underbrace{\bar o\,(1-\bar o)}_{	\text{uncertainty}},
\end{equation}
groups forecasts into bins $k$ (with $n_k$ cases, mean forecast $\bar p_k$, and
observed frequency $\bar o_k$, and overall base rate $\bar o$): the first term is
reliability (calibration), the second resolution (informativeness), and the third
the irreducible uncertainty common to all forecasts. We report the Brier skill
score against climatology together with the resolution term, since a
calibrated-but-uninformative forecast (e.g.\ climatology) has zero resolution: a
useful model must therefore clear the bar of positive resolution while retaining
good reliability.

\paragraph{Whole distribution and tail: (tw)CRPS.} We score the predictive
distribution with the CRPS \citep{gneiting2007scoring} and its
...
quantile regression \citep{koenker1978quantile} whose regularisation is chosen by cross-validation on the
...
reference (the empirical training distribution), and every estimate carries a
moving-block bootstrap \citep{kunsch1989block} confidence interval ($1{,}000$--$2{,}000$
resamples, blocks of ten extreme days to respect short-range temporal
dependence). The intensity result is further replicated across the four
forward-only, expanding-window splits of Section~\ref{sec:data-split} \emph{and
across the two cities}.

The block length deserves its own check. It is not tightly identified by the
short-range autocorrelation of Figure~\ref{fig:data}d alone, and the smallest
split has only $96$ test extremes in London (roughly ten blocks of ten). We
therefore recomputed the interval at block lengths of $5$ and $20$ at that
split: the $90\%$ CI moves by at most $\pm0.003$ around the reported
$[+0.006,+0.059]$ in London and $[+0.019,+0.073]$ in Paris
(Table~\ref{tab:intensity}), so the block-$10$ choice is not load-bearing in the
smallest-sample case.

Finally, per-feature significance is controlled for multiplicity by the
Benjamini--Hochberg false-discovery rate; feature importance is quantified by
stability selection (selection frequency under subsampling); and the GP--EVT
hurdle is probed by a sensitivity sweep over inducing points. Discrimination
metrics (precision--recall AUC, PR-AUC) are reported only as secondary context,
and conformal coverage is reported but, per Section~\ref{sec:conformal}, never as
evidence of predictive quality.
...
skill score ($+0.037$ in London, $+0.123$ in Paris; bootstrap intervals
...
(twCRPS skill $-2.3$ in London), whereas the light-tailed gamma and the
...
delicate configuration; it is about as flexible as this low-signal
problem can bear.
...
is worse than climatology on the proper scores (Table~\ref{tab:bakeoff}, twCRPS
...
Occurrence is weakly but genuinely predictable in both cities: the calibrated logistic regression improves on climatology with a Brier skill of $+0.038$ in London and
...
antecedent predictors --- led by the {K-index} of convective instability,
...
quantile grid across four successive expanding-window splits per city: the skill
stays positive throughout --- every interval excluding zero (though in the two smallest windows the interval only marginally clears zero, $P(>0)=0.97$--$0.99$), with primary-split
...
single runs. The same decay and flat coverage hold in Paris (Figure~\ref{fig:horizon}c,d).
...
climatology that excludes zero across four successive splits
...
Predictability is real but small, and physically interpretable.
The central result is not strong forecast skill but a careful measurement of how much skill is genuinely present: in both
...
as many extremes --- the extended GPD~\citep{davison1990,coles2001evt} does, so whether the tail is climatology-bound
is a sample-size effect rather than a universal law.

Three methodological lessons are worth emphasising. First,
...
inexpensive logistic regression and gradient-boosted trees --- so added model
flexibility yielded little benefit, and a modest Gaussian process is about as
flexible as this low-signal problem can bear. Gaussian processes would
offer greater benefit through structure that the single-site temporal problem does
...
coverage is insufficient as a stand-alone indicator of forecast quality}~\citep{gneiting2007scoring,lerch2017dilemma}: it is
...
interval excludes zero), with negligible interaction. The only way
...
Several apparent results did not survive
...
scoring, cross-validated model selection, bootstrap intervals~\citep{kunsch1989block}, and multi-split
...
Four limitations qualify these conclusions. First, the assessment covers only two cities. Although this is a step beyond a single site, London and Paris share a climate zone, a downscaled reanalysis product (ERA5-2km), and a common data provider, so they are not fully independent replicates and shared reanalysis biases could inflate their apparent skill together; replication in a climatically distinct setting is the most direct next step. Second, the index is itself an ERA5-derived precipitation product, which raises a circularity concern; we mitigate it by excluding all precipitation fields from the predictors and lagging every driver, so that only antecedent dynamical and thermodynamic state informs the forecast, though a residual dependence through the reanalysis cannot be excluded entirely. Third, the study uses a perfect-prognosis design based on antecedent reanalysis fields rather than operational forecasts, and the intensity analysis conditions on observed exceedances; it therefore isolates predictability rather than delivering an end-to-end warning, and moving to numerical-weather-prediction drivers is a natural extension. Fourth, we treat the 1989--2018 climatological reference as stationary; given that projections indicate a rising trend in extreme rainfall over this period \citep{ipcc2021}, a slowly time-varying baseline could sharpen the reported skill scores, whose likely direction is a modest downward bias in later test years.
...
precipitation extremes using an evaluation protocol designed for low-signal
settings. The answer to how such predictability should be assessed is
methodological: calibration-aware comparisons of proper scores against
climatological benchmarks reveal differences among models, and between
occurrence and full-distribution skill, that conformal coverage alone cannot.
Although all models achieved nominal coverage, only proper-score comparisons
identified which forecasts genuinely contained predictive information.

The answer to how predictable the extremes themselves are is more qualified:
there is modest but measurable predictability. The occurrence and magnitude of
the largest events are weakly but consistently informed by antecedent
convective instability, with a compact Gaussian-process extreme-value hurdle
model providing the strongest full-distribution forecasts in the tail, while
simpler calibrated models perform similarly for occurrence alone. The
consistency of these findings across the more convective Paris and the maritime
London suggests that the identified signal is not specific to a single location,
while its stronger expression in Paris highlights the influence of convective
regimes. For the flood managers and insurers who motivated this study,
antecedent conditions therefore provide a small but genuine and honestly
calibrated source of predictive information.

Natural extensions are those in which additional signal, and a stronger role
for structured Gaussian-process models, are most plausible: expanding to more
cities to characterise predictability across climates, introducing spatial
dependence across grids of cells, incorporating operational drivers from
numerical weather prediction rather than reanalysis, and considering
sub-seasonal aggregations where slowly varying predictors may contribute.
Whether predictability increases materially in these settings remains, in the
spirit of this paper, a question to be answered by proper scores and uncertainty
quantification rather than asserted.
...
\end{equation}
where $\KL$ is the Kullback--Leibler divergence and the expectation is
computed by Monte Carlo over the marginal $q(\bg(\bx_{t-h}))$.
...
cities. Brier skill score (BSS) and twCRPS skill are relative to
...
model on the tail in both cities. A dash (---) marks a model with no full predictive distribution (occurrence-only). The zero-inflated log-normal's twCRPS ($\approx -2.3$ in London) reflects a calibration failure and is off the scale of the other entries.}
\label{tab:bakeoff}
\begin{tabular}{lcccc}
\toprule
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_reliability}
\caption{Reliability diagrams for the exceedance probability at the $95$th percentile ($h{=}1$) for \textbf{(a)}~London and \textbf{(b)}~Paris. In both cities the logistic regression and the GP--EVT hurdle track the diagonal (calibrated) and spread their predictions (informative); the zero-inflated log-normal is systematically overconfident (its predicted exceedance probabilities exceed the observed frequencies), despite attaining nominal conformal coverage by construction. Climatology is a single point at the base rate.}
...
\caption{Seed-averaged horizon sweep on the $95$th-percentile index (GP--EVT hurdle) for London \textbf{(a,b)} and Paris \textbf{(c,d)}. In panels \textbf{(a,c)} the occurrence PR-AUC decays toward the prevalence floor (dashed) as the forecast horizon grows; the $+$ERA5 driver gain is clearest at $h=1$ and fades into seed noise by $h=3$. In panels \textbf{(b,d)} the conformal coverage stays at the nominal $0.90$ across all horizons and predictor sets. Error bars / markers are means over three seeds. The same decay and flat coverage hold in both cities.}
\label{fig:horizon}
\end{figure}
