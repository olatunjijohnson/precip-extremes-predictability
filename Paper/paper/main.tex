...
(1989--2018) of a standardised daily precipitation-extreme index for two
climatically contrasting cities, London (maritime) and Paris (more continental),
...
bake-off of models from logistic regression and gradient-boosted trees to a deep
Gaussian-process extreme-value hurdle, we find --- consistently in both cities ---
that (i) \textbf{simple calibrated models outperform the elaborate ones}, and the
choice of response distribution dominates model class; (ii) \textbf{occurrence} of
an extreme is weakly but genuinely predictable from antecedent state (led by
convective-instability and circulation indices); and (iii) \textbf{the magnitude
of the largest events} carries weak but robust predictability tied to antecedent
convective instability (CAPE/CIN), with a continuous-ranked-probability skill over
climatology whose moving-block bootstrap interval excludes zero across four
independent train/test splits in each city. The effects are small but
statistically robust, physically plausible, and replicated across both London
and Paris, suggesting that they are unlikely to be a location-specific artefact.
\end{abstract}
...
\begin{enumerate}[leftmargin=1.6em,itemsep=1pt]
  ...
  each city --- and is stronger in the more convective Paris, indicating the
  signal is not location-specific.
...
(Paris is qualitatively similar).
...
is weakly identified, so we first confirm on synthetic data that the mechanism
is recoverable and beneficial \emph{when the dependence genuinely exists}
(Section~\ref{sec:res-synth}); this isolates whether any real-data shortfall is a
failure of the model or an absence of signal. This construction is related to
correlated two-part models in biostatistics and econometrics
...
of Section~\ref{sec:data-split} \emph{and across the two cities}. Per-feature
...
most recent index lag are selected in essentially every training subsample.
Skill nonetheless decays toward the prevalence floor as the forecast horizon grows
(Figure~\ref{fig:horizon}a), consistent with the limited day-scale persistence of
...
climatological excess distribution. The CRPS skill is small but its moving-block
bootstrap interval \emph{excludes zero} in each city (London $+0.014$,
$[+0.009,+0.019]$; Paris $+0.036$, $[+0.005,+0.074]$), and it remains positive
across four independent expanding-window splits per city (Table~\ref{tab:intensity},
Figure~\ref{fig:intensity}), with every interval excluding zero. The signal is
concentrated in the upper tail (the largest events) and is carried by antecedent
convective instability (CAPE/CIN); median magnitude is indistinguishable from
climatology. As with occurrence, intensity predictability is stronger in the more
convective Paris.
...
interval excludes zero in every split and city, and is larger in Paris.}
\label{fig:intensity}
\end{figure}
...
moving-block bootstrap intervals. Every interval excludes zero; the effect is
larger in Paris.}
\label{tab:intensity}
\begin{tabular}{lcccc}
\toprule
...
matters most exactly where intensity data are scarcest. The implication for the
real-data results is the important one: the coupled model is recoverable and
beneficial \emph{when} occurrence and intensity are genuinely dependent, so its
failure to improve on the real index (Section~\ref{sec:res-bakeoff}) reflects an
absence of exploitable dependence in the data, not a defect of the model class
--- reinforcing that the shortfall is one of signal, not of method.
...
conclusions holds in \emph{both} London and Paris, and is quantitatively stronger
in the more convective Paris --- evidence that they reflect the meteorology of
precipitation extremes rather than the idiosyncrasies of one city.
...
components through shared latent structure --- which is precisely the dependent
deep-GP hurdle we tested and found wanting, reinforcing that the parsimonious,
separable treatment is the right one here.
...
results as a careful two-city assessment rather than a continent-wide claim. It
...
isolates predictability rather than delivering an end-to-end warning. Finally, the
...
finding holds --- and strengthens --- in the more convective Paris is itself
evidence that these are properties of precipitation extremes, not of one city.
...
cities, are available in a repository made public on acceptance
(\textsc{[repository url / archived doi to be inserted]}).
